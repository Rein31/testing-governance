...
let governance;
    ...
    it('Check voting period', async () => {
        ...
        assert.equal(votingPeriod.toString(), 5, "Voting Period error")
    })
    ...
    it('Create a new proposal and casting vote', async () => {
        ...
        // await token.transfer(voter1, amount, { from: accounts[0] })
        let amount = web3.utils.toWei('50', 'ether');
        await token.transfer(accounts[6], amount, { from: accounts[0] })
        await token.transfer(accounts[7], amount, { from: accounts[0] })
        await token.transfer(accounts[8], amount, { from: accounts[0] })
        await token.transfer(accounts[9], amount, { from: accounts[0] })
        ...
        const tx = await governance.propose([token.address], [0], [encodedFunction], description, { from: accounts[3]});

        const id = tx.logs[0].args.proposalId;

        let proposalState = await governance.state.call(id)
        ...
        // console.log(`Current blocknumber: ${blockNumber.number}\n`)

        vote = await governance.castVote(id, 1, { from: accounts[0] })
        vote = await governance.castVote(id, 1, { from: accounts[6] })
        vote = await governance.castVote(id, 1, { from: accounts[7] })
        vote = await governance.castVote(id, 0, { from: accounts[8] })
        vote = await governance.castVote(id, 2, { from: accounts[9] })
